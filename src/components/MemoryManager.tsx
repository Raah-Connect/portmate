...
export function MemoryManager({ ship, onRestart }: Props) {
  ...
  useEffect(() => {
    ...
    listen<MemoryOpDonePayload>("memory-op-done", (event) => {
      ...
      if (event.payload.success) {
        setPhase("done");
        setMessage(`${event.payload.op} complete — restarting ship…`);
        setTimeout(() => {
          setPhase("idle");
          setActiveOp(null);
          setMessage("");
  ...
  return (
    <div style={wrapStyle}>
      ...
      {/* Hint */}
      <div style={hintStyle}>
        All ops stop the ship, run the binary, then restart automatically. Check the terminal for progress. Run chop after roll.
      </div>
    </div>
  );
}
